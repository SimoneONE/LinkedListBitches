...
int set_buffer_size(int fd){
	...
	printf("new size = ");
	new_size=input_int();
	if(ioctl(fd,LL_SET_MAX_SIZE,&new_size)==-1)
		return -1;	
	else
...
int get_buffer_size(int fd){
	int size;
	if(ioctl(fd,LL_GET_MAX_SIZE,&size)==-1)
		return -1;
	else
		printf("size get : %d\n",size);
	return 0;
...
int set_packet_max_size(int fd){
	...
	printf("new size = ");
	new_size=input_int();
	if(ioctl(fd,LL_SET_PACK_MAX_SIZE,&new_size)==-1)
		return -1;	
	else
...
int get_packet_max_size(int fd){
	int size;
	if(ioctl(fd,LL_GET_PACK_MAX_SIZE,&size)==-1)
		return -1;
	else
...
int set_packet_min_size(int fd){
	...
	printf("new size = ");
	new_size=input_int();
	if(ioctl(fd,LL_SET_PACK_MIN_SIZE,&new_size)==-1)
		return -1;	
	else
...
int get_packet_min_size(int fd){
	int size;
	if(ioctl(fd,LL_GET_PACK_MIN_SIZE,&size)==-1)
		return -1;
	else
